...
public class Parser {
    ...
    private Optional<ExpressionNode> getRightOfAndTerm(ExpressionNode leftNode) throws SyntaxErrorException {
        ...
        if (rightNode.isEmpty())
            throw new SyntaxErrorException("Found && but not Boolean factor after", tokenManager.getCurrentLine(), tokenManager.getCurrentColumnNumber(), tokenManager.getCurrentFileName());
    ...
    // This Method parses both methods and member accesses that are chained together using ParseChainedMethodCallExpression
    ...
    private Optional<ExpressionNode> MethodCallExpression() throws SyntaxErrorException {
        ...
        // Name. (Get the first name of the expression
        tokenManager.matchAndRemove(Token.TokenTypes.DOT);
        // If the next part is a method call and not a member reference (method())
        if (tokenManager.nextTwoTokensMatch(Token.TokenTypes.WORD, Token.TokenTypes.LPAREN)) {
            var mce = new MethodCallExpressionNode();
            mce.object = Optional.of(new VariableReferenceNode() {{
                this.name = nameToken.get().getValue();
            }});
            ...
            return ParseChainedMethodCallExpression(mce);
        } else { // Name.name (Member access (class.member))
            var memberName = tokenManager.matchAndRemove(Token.TokenTypes.WORD);
            if (memberName.isEmpty())
                throw new SyntaxErrorException("identifier expected", tokenManager.getCurrentLine(), tokenManager.getCurrentColumnNumber(), tokenManager.getCurrentFileName());
            var memberExpression = new MemberExpressionNode(new VariableReferenceNode() {{
                this.name = nameToken.get().getValue();
            }}, new VariableReferenceNode() {{
                this.name = memberName.get().getValue();
            }});
            return ParseChainedMethodCallExpression(memberExpression);
        }
    }

    // Helper method to recursively parse method calls and member references that are chained together
